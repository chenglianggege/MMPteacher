    ...
    <div class="question-detail" v-show="!showCart || showDetail">
      ...
      <div class="question-prediv">
        <div class="qs-info" v-for="(info,infoIdx) in question.info" :key="info.info_id">
          ...
          <img class="option-img picimg_cl" mode="aspectFit" v-if="question.info[0].info_content_img&&infoIdx==0" :src="STATIC_URL + question.info[0].info_content_img">
          <div class="items" v-for="(item,itemIdx) in info.items" :key="item.item_id">
            ...
            <div v-if="infoIdx==0" class="item-answer">第{{itemIdx+1}}题 题目满分：{{item.item_score}}分 </div>
            <div v-if="infoIdx==1" class="item-answer">准备时间：{{item.item_prepare_second}}秒 答题时间：{{item.item_answer_second}}秒 题目满分：{{item.item_score}}分 </div>
