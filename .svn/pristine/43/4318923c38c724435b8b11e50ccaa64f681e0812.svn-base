      ...
      <div class="question-prediv">
        <div class="qs-info" v-for="(info,infoIdx) in question.info" :key="info.info_id">
          <div class="info-content">
            短文录音：<icon v-if="info.source_content" class="iconfont icon-laba" :class="{'onplay': playUrl == info.source_content}" @click="audioPlay(info.source_content)"></icon>
          </div>
          <img class="info-img" mode="aspectFit" v-if="info.info_content_img" :src="STATIC_URL + info.info_content_img">
